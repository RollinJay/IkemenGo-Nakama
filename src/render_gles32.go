...
func (r *Renderer_GLES32) SetViewport(x, y, width, height int32) {
	// System coordinates use a top-left origin; OpenGL uses bottom-left.
	realY := sys.presentation.canvas[3] - (y + height)
	gl.Viewport(x, realY, width, height)
}

func (r *Renderer_GLES32) EnableScissor(x, y, width, height int32) {
	// Scissor coordinates are local to the current render surface.
	x += sys.presentation.origin[0]
	realY := sys.presentation.canvas[3] - (sys.presentation.origin[1] + y + height)

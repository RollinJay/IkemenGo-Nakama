...
func (f *Font_VK) Printf(x, y float32, xscl, yscl float32, spacingXAdd float32, align int32, blend bool, window [4]int32,
	...
	view := sys.renderViewport()
	viewports := []vk.Viewport{{
		MinDepth: 0.0,
		MaxDepth: 1.0,
		X:        float32(view[0]),
		Y:        float32(view[1]),
		Width:    float32(view[2]),
		Height:   float32(view[3]),
	}}
	scissors := []vk.Rect2D{{
		Offset: vk.Offset2D{
			X: int32(sys.presentation.origin[0]) + int32(Max(int(window[0]), 0)),
			Y: int32(sys.presentation.origin[1]) + int32(Max(int(window[1]), 0)),
		},
		Extent: vk.Extent2D{

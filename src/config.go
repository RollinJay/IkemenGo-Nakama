...
type Config struct {
	...
	Video struct {
		...
		Border     struct {
			...
			Height      int32    `ini:"Height"`
			GameOffsetX int32    `ini:"GameOffsetX"`
			GameOffsetY int32    `ini:"GameOffsetY"`
	...
	Netplay struct {
		...
		Ranked          struct {
			BestOf               int  `ini:"BestOf" sync:"host"`
			SwitchSides          bool `ini:"SwitchSides" sync:"host"`
			WinnerKeepsSelection bool `ini:"WinnerKeepsSelection" sync:"host"`
		} `ini:"Ranked"`
	} `ini:"Netplay"`
	Input struct {
...
func (c *Config) normalize() {
	...
	if c.Video.Border.Height < 1 {
		c.SetValueUpdate("Video.Border.Height", c.Video.GameHeight)
	}
	if c.Video.Border.Localcoord[0] < 1 || c.Video.Border.Localcoord[1] < 1 {
		c.SetValueUpdate("Video.Border.Localcoord", [2]int32{c.Video.Border.Width, c.Video.Border.Height})
	}
	bestOf := c.Netplay.Ranked.BestOf
	if bestOf < 1 {
		bestOf = 3
	}
	if bestOf%2 == 0 {
	...
	c.SetValueUpdate("Netplay.Ranked.BestOf", bestOf)
...
func (c *Config) sysSet() {
	...
	if _, ok := sys.cmdFlags["-bordered"]; ok {
		c.Video.Border.Enabled = true
	}
	if v, ok := sys.cmdFlags["-borderwidth"]; ok {
		if n, err := strconv.ParseInt(v, 10, 32); err == nil && n > 0 {
			c.Video.Border.Width = int32(n)
		}
	}
	if v, ok := sys.cmdFlags["-borderheight"]; ok {
		if n, err := strconv.ParseInt(v, 10, 32); err == nil && n > 0 {
			c.Video.Border.Height = int32(n)
		}
	}
	if v, ok := sys.cmdFlags["-bordergamex"]; ok {
		if n, err := strconv.ParseInt(v, 10, 32); err == nil {
			c.Video.Border.GameOffsetX = int32(n)
		}
	}
	if v, ok := sys.cmdFlags["-bordergamey"]; ok {
		if n, err := strconv.ParseInt(v, 10, 32); err == nil {
			c.Video.Border.GameOffsetY = int32(n)
		}
	}

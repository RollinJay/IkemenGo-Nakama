...
type System struct {
	...
	nakama                         *NakamaClient
	nakamaSpectatorLoading         bool
	nakamaSpectatorLoadingText     *TextSprite
	nakamaSpectatorLoadingTitle    *TextSprite
	nakamaSpectatorLoadingProgress *TextSprite
	nakamaSpectatorLoadingDetail   *TextSprite
...
func (s *System) init(w, h int32) *lua.LState {
	...
	s.setGameSize(w, h)
	s.configurePresentation(w, h)
	// Keep scrrect as the outer presentation canvas between render passes.
	s.scrrect = s.presentation.canvas
	s.presentation.origin = [2]int32{0, 0}
	s.restoreAspectState(s.outerRenderState())
	for i := range sys.cgi {
...
func (s *System) applyFightAspect() {
	...
	// Scale to the physical game viewport when a separate presentation canvas is active.
	viewport := s.presentationGameRect()
	if s.presentationEnabled() {
		s.widthScale = float32(viewport[2]) / float32(s.gameWidth)
		s.heightScale = float32(viewport[3]) / float32(s.gameHeight)
	} else {
...
func (s *System) renderFrame() {
	...
	if !s.presentationEnabled() {
		...
		if s.nakamaSpectatorLoading {
			s.luaDiscardDrawQueue()
			s.drawNakamaSpectatorLoading()
			return
		}
	...
	// Border mode uses an outer presentation canvas and a separate game render
	// surface. The outer surface remains intact while the game is rendered into
	// its own viewport, so border pixels are never part of gameplay state.
	outer := s.captureRenderSurfaceState()
	s.beginRenderSurface(s.presentation.canvas, s.outerRenderState())
	s.motif.drawBorderBackground()
	if s.luaLState != nil {
		if err := s.luaLState.DoString("hook.run('game.border_bg')"); err != nil {
			s.luaLState.RaiseError("Error executing Lua hook: %s\n%v", "game.border_bg", err)
		}
		s.luaFlushDrawQueue()
	}

	gameRect := s.presentationGameRect()
	gameDrawState := logicState
	if s.shouldComposeFullResolution() {
		gameDrawState = aspectStateForSize(gameRect[2], gameRect[3])
	}
	s.beginRenderSurface(gameRect, gameDrawState)
	if s.nakamaSpectatorLoading {
		s.luaDiscardDrawQueue()
		s.drawNakamaSpectatorLoading()
	} else {
...
func (s *System) beginNakamaSpectatorLoading() {
	if s == nil {
		return
	}
	s.nakamaSpectatorLoading = true
	s.nakamaSpectatorLoadingText = nil
	s.nakamaSpectatorLoadingTitle = nil
	...
	if baseLoading.TextSpriteData != nil {
		ts := baseLoading.TextSpriteData.Copy()
		ts.text = baseLoading.Text
		ts.textInit = baseLoading.Text
		ts.textDelay = 0
		ts.Reset()
	...
	s.nakamaSpectatorLoadingProgress = buildText(&loading.Progress, loading.Progress.Text)
	s.nakamaSpectatorLoadingDetail = buildText(&loading.Detail, loading.Detail.Text)
}

func (s *System) endNakamaSpectatorLoading() {
	if s == nil {
		return
	}
	s.nakamaSpectatorLoading = false
	s.nakamaSpectatorLoadingText = nil
	s.nakamaSpectatorLoadingTitle = nil
	s.nakamaSpectatorLoadingProgress = nil
	s.nakamaSpectatorLoadingDetail = nil
...
func (s *System) updateNakamaSpectatorLoading() bool {
	if !s.nakamaSpectatorLoading || s.nakama == nil {
		return false
	}
	for _, ts := range []*TextSprite{
		s.nakamaSpectatorLoadingTitle,
		s.nakamaSpectatorLoadingText,
		s.nakamaSpectatorLoadingProgress,
		s.nakamaSpectatorLoadingDetail,
	} {
		if ts != nil {
			ts.Update()
		}
	...
	if reason := buffer.ResetReason(); reason != "" {
		LogMessage("Nakama spectator replay reset: %s", reason)
	...
	if err := s.beginReplaySession(rf); err != nil {
		rf.Close()
		LogMessage("Nakama spectator replay synchronization failed: %v", err)
		s.endNakamaSpectatorLoading()
		return true
	}
	s.sessionWarning = ""
...
func (s *System) drawNakamaSpectatorLoading() {
	// Screenpacks may provide a dedicated [NakamaBgDef]. Without one, retain
	// IKEMEN's existing neutral loading clear.
	bg := &s.motif.NakamaBgDef
	if s.motif.NakamaInfo.Enabled && bg != nil && bg.BGDef != nil {
		if bg.BgClearColor[0] >= 0 && bg.RectData != nil {
			bg.RectData.Draw(0)
		}
		bg.BGDef.step()
		bg.BGDef.Draw(bg.StartLayer, 0, 0, 1)
	} else {
		FillRect(s.scrrect, 0x000000, [2]int32{255, 0}, nil)
	}
	...
	drawText := func(ts *TextSprite, text string) {
		if ts == nil {
			return
		}
		ts.text = text
	...
	if strings.TrimSpace(loadingText) == "" {
		loadingText = s.motif.TitleInfo.Loading.Wait.Text
	}
	if !s.motif.NakamaInfo.Enabled {
		if s.nakamaSpectatorLoadingText != nil {
			drawText(s.nakamaSpectatorLoadingText, s.motif.TitleInfo.Loading.Wait.Text)
		}
	} else {
		if s.nakamaSpectatorLoadingTitle != nil {
			drawText(s.nakamaSpectatorLoadingTitle, s.motif.NakamaInfo.Title.Text)
		}
		if s.nakamaSpectatorLoadingText != nil {
			drawText(s.nakamaSpectatorLoadingText, loadingText)
		}
	}
	if s.motif.NakamaInfo.Enabled && s.nakamaSpectatorLoadingProgress != nil && delay > 0 {
		percent := int64(buffered) * 100 / int64(delay)
		if percent > 100 {
			percent = 100
		}
		drawText(s.nakamaSpectatorLoadingProgress, fmt.Sprintf(loading.Progress.Text, percent))
	}
	if s.nakamaSpectatorLoadingDetail != nil && delay > 0 {
		remaining := delay - buffered
		if remaining < 0 {
			remaining = 0
		}
		seconds := int64(remaining) / int64(replayStreamFPS)
		drawText(s.nakamaSpectatorLoadingDetail, fmt.Sprintf(loading.Detail.Text, seconds))
	}
}
...
func (s *System) update() bool {
	...
	if s.nakamaSpectatorLoading {
		if !s.updateNakamaSpectatorLoading() {

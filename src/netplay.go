...
func readReplayHeader(f *os.File) (*ReplayHeader, error) {
	...
	const maxReplayHeaderSize = 4 << 20
	if size == 0 || int64(size) > maxReplayHeaderSize {
		return nil, fmt.Errorf("invalid replay header size: %d", size)
	}
	body := make([]byte, size)
...
func NewLiveReplayFile(buffer *NakamaReplayBuffer) (*ReplayFile, error) {
	if buffer == nil {
		return nil, errors.New("nil live replay buffer")
	}
	header := buffer.Header()
	if header == nil {
		return nil, errors.New("live replay header has not been received")
	}
	if header.InputCount != REPLAY_NUM_INPUTS || header.InputBytes != REPLAY_INPUT_BYTES {
		return nil, fmt.Errorf("unsupported live replay input format: count=%d bytes=%d", header.InputCount, header.InputBytes)
	}
	rf := &ReplayFile{
		liveBuffer:         buffer,
		strictSettings:     nil,
	...
	if header.ReplayHeader != nil {
		rf.strictSettings = cloneSyncSettings(header.ReplayHeader.Strict)
		rf.hostSettings = cloneSyncSettings(header.ReplayHeader.Host)
		rf.contentFingerprint = header.ReplayHeader.ContentFingerprint
	}
	return rf, nil
}
...
func validateStrictCompatibility(local, remote []SyncSetting, replayMode bool) error {
	...
	if len(localMap) != len(remoteMap) {
		return Error("Sync strict config schema mismatch")
	}
...
func (s *System) currentContentFingerprint() string {
	...
	writeFile := func(label, filename string) {
		if strings.TrimSpace(filename) == "" {
			writeField(label+":state", "absent")
			return
		}
		f, err := OpenFile(filename)
		if err != nil {
			log.Printf("Content fingerprint could not open %q: %v", filename, err)
			writeField(label+":state", "open-error")
			return
		}
		defer f.Close()
		if _, err := io.Copy(h, f); err != nil {
			log.Printf("Content fingerprint could not read %q: %v", filename, err)
			writeField(label+":state", "read-error")
			return
		}
		writeField(label+":state", "ok")
	}

	writeField("engine.version", Version)
	writeField("sync.version", strconv.Itoa(int(syncConfigVersion)))

	// These definitions establish the active engine/match environment before
	// rollback starts. Hash their contents rather than their absolute filesystem
	// locations so two installations can agree on identical content.
	...
	for i, filename := range commonFX {
		writeFile(fmt.Sprintf("common.fx.%d", i), filename)
	}

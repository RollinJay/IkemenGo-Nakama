...
func realMain() {
	...
	// Optional dedicated-server tools stay outside SDL/game initialization.
	if _, ok := sys.cmdFlags["-server-wizard"]; ok {
		path := defaultNakamaServerConfigPath
		if val, exists := sys.cmdFlags["-server-config"]; exists && strings.TrimSpace(val) != "" {
			path = val
		}
		if err := runServerWizard(os.Stdin, os.Stdout, path); err != nil {
			fmt.Fprintln(os.Stderr, "Server wizard error:", err)
		}
		return
	}
	if _, ok := sys.cmdFlags["-server"]; ok {
...
-server                  Starts the optional dedicated Nakama server
-server-wizard           Runs the headless dedicated-server setup wizard

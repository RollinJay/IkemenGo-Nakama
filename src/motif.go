...
// BorderLayerProperties reuses the engine's existing animation, text, and
// rectangle primitives so border UI can be authored like other motif UI.
type BorderLayerProperties struct {
	AnimationProperties
	Text    AnimationTextProperties `ini:"text"`
	Overlay OverlayProperties       `ini:"overlay"`
}

type BorderInfoProperties struct {
	Enabled    bool                  `ini:"enabled"`
	Localcoord [2]int32              `ini:"localcoord"`
	Background BorderLayerProperties `ini:"background"`
	Foreground BorderLayerProperties `ini:"foreground"`
}
...
type NakamaInfoProperties struct {
	Enabled bool           `ini:"enabled"`
	FadeIn  FadeProperties `ini:"fadein"`
	FadeOut FadeProperties `ini:"fadeout"`
	Title   TextProperties `ini:"title"`
	Loading struct {
		AnimationTextProperties
		Progress TextProperties `ini:"progress"`
		Detail   TextProperties `ini:"detail"`
	} `ini:"loading"`

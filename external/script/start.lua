...
start.challenger = 0
-- Nakama online rematch state. These values are consumed by the mode loop after
-- the current rollback session has ended.
start.onlineNextAction = nil
start.onlineRematchSnapshot = nil
start.onlineLastFightSnapshot = nil
start.onlineP2PRequested = false
start.onlineForfeitUserId = nil

local function onlineStunServers()
	local stun = getCommandLineValue('-nakama-stun')
	if stun ~= nil and stun ~= '' then return stun end
	if nakama ~= nil then
		-- The rematch module owns its optional STUN list; return an empty string when
		-- it is not loaded so the caller can still use host candidates.
		local ok, cfg = pcall(function() return ik_rematch and ik_rematch.sd and ik_rematch.sd.rematchonline end)
		if ok and cfg ~= nil and cfg.p2p ~= nil then
			return cfg.p2p.stunservers or ''
		end
	end
	return ''
end

function start.f_startOnlineP2P()
	if nakama == nil or nakama.currentMatch == nil or nakama.currentMatch() == '' then
		return false
	end
	if nakama.p2pReady ~= nil and nakama.p2pReady() then
		return true
	end
	if nakama.startP2P == nil then return false end
	local ok, err = pcall(nakama.startP2P, gameOption('Netplay.Rollback.Port'), onlineStunServers())
	if not ok then
		print('Nakama P2P start failed: ' .. tostring(err))
		return false
	end
	return true
end

function start.f_waitOnlineP2P(timeoutFrames)
	if nakama == nil or nakama.currentMatch == nil or nakama.currentMatch() == '' then
		return false
	end
	if nakama.p2pReady == nil then return false end
	if nakama.p2pReady() then return true end
	local limit = timeoutFrames or 300
	for _ = 1, limit do
		if esc() then return false end
		if nakama.currentMatch() == '' then return false end
		if nakama.p2pReady() then return true end
		refresh()
	end
	return nakama.p2pReady()
end
...
--sets stage
function start.f_setStage(num, assigned, forceStage)
	-- Online rematches already negotiated the exact stage during the previous
	-- versus screen. Honor it before the normal stage-menu/random-stage logic.
	if forceStage and num ~= nil then
		selectStage(num)
...
function start.f_game(common)
	...
	start.onlineForfeitUserId = nil
	-- Ranked sets are maintained outside rollback state. A server-resolved forfeit
	-- supersedes the gameplay winner and ends the current set immediately.
	if forfeitUser ~= nil and forfeitUser ~= '' and nakama ~= nil and nakama.rankedSet ~= nil then
		local rsBefore = nakama.rankedSet()
		if rsBefore.active then
			local player = 0
			if rsBefore.p1_id == forfeitUser then
				player = 1
			elseif rsBefore.p2_id == forfeitUser then
				player = 2
			end
			if player > 0 and nakama.rankedSetForfeit ~= nil and nakama.rankedSetForfeit(player) then
				rankedAfter = nakama.rankedSet()
				start.rankedCarry = nil
				start.rankedSetFinished = true
			else
				print('Nakama ranked forfeit could not be mapped to a set player: ' .. tostring(forfeitUser))
			end
		end
	end
	-- Ranked sets are maintained outside rollback state. Capture each player's
	-- confirmed team before launchFight clears selection state, then record the
	-- physical winner side. The saved selections follow player identity when the
	-- next match swaps sides.
	if not start.rankedSetFinished and winner >= 1 and winner <= 2 and nakama ~= nil and nakama.rankedSet ~= nil then
		local rsBefore = nakama.rankedSet()
		if rsBefore.active then
			local carry = {}
			for player = 1, 2 do
				local side = rsBefore['p' .. player .. '_side']
				local ps = start.p[side]
				carry[player] = {
					selected = main.f_tableCopy(ps.t_selected or {}),
					teamMode = ps.teamMode,
					numChars = ps.numChars,
				}
			end
			local complete = nakama.rankedSetRecordMatch(winner)
			rankedAfter = nakama.rankedSet()
			if complete then
				start.rankedCarry = nil
				start.rankedSetFinished = true
				...
				if rankedAfter.winner_keeps_selection then
					start.rankedCarry = carry
				else
					start.rankedCarry = nil
				end
			end
	...
	main.f_restoreInput()
	if rankedAfter ~= nil and rankedAfter.active and not start.rankedSetFinished and rankedAfter.switch_sides then
		-- Keep local controller identities attached to the same set players when
		-- odd/even player slots exchange physical sides. This must happen after
		-- the normal post-match input restoration.
		for pn = 1, gameOption('Config.Players') - 1, 2 do
			local a = getRemapInput(pn)
			local b = getRemapInput(pn + 1)
			remapInput(pn, b)
			remapInput(pn + 1, a)
		end
	end
...
function start.f_selectMode()
	...
	while true do
		-- Online rematch decisions are resolved by the Nakama coordination match.
		-- A decision is consumed here, after the completed game has returned, so
		-- the same coordination session can be reused while each GGPO gameplay
		-- session gets a fresh P2P socket.
		if start.onlineNextAction == 'exit' or start.onlineNextAction == 'forfeit' then
		...
		elseif start.onlineNextAction == 'rematch' then
			...
			if start.f_prepareOnlineRematchSnapshot ~= nil then
				snap = start.f_prepareOnlineRematchSnapshot(snap)
			end
			start.f_clearOnlineRematchState()
			start.onlineP2PRequested = false
			if not start.f_startOnlineP2P() then
			...
			if type(snap) ~= 'table' or type(snap.p1) ~= 'table' or type(snap.p2) ~= 'table' then
				panicError('Nakama rematch snapshot is invalid; returning to online menu.')
				start.f_leaveOnlineSession()
				return
			end
			...
			local rematchData = {
				...
				p1char = snap.p1,
				p2char = snap.p2,
				p1teammode = snap.p1teammode,
				p2teammode = snap.p2teammode,
				p1numchars = #snap.p1,
				p2numchars = #snap.p2,
				stageNo = snap.stageNo,
				forceStage = true,
				victoryscreen = main.motif.victoryscreen,
				winscreen = false,
			}
			if not launchFight(rematchData) then
				start.f_leaveOnlineSession()
				return
			end
			if start.exit then
...
function launchFight(data)
	local data = data or {}
	local t = {}
	if continued() and not data.onlineSession then -- normal continue; online rematch supplies its own session data
	...
	while true do
		...
		t.stageNo = start.f_setStage(t.stageNo, t.stageAssigned or continued() or loopCount > 0, t.forceStage)
		...
		if start.rankedSetFinished and start.onlineNextAction == nil then
			start.rankedSetFinished = false
			start.exit = true
		end
...
local function applyRankedCarrySelection()
	if start.rankedCarry == nil or nakama == nil or nakama.rankedSet == nil then
		return
	end
	local rs = nakama.rankedSet()
	local winner = rs.last_winner or 0
	if not rs.active or not rs.winner_keeps_selection or winner < 1 or winner > 2 then
		start.rankedCarry = nil
		return
	end
	local side = rs['p' .. winner .. '_side']
	local saved = start.rankedCarry[winner]
	if side < 1 or side > 2 or saved == nil then
		start.rankedCarry = nil
		return
	end
	start.p[side].teamMode = saved.teamMode
	start.p[side].numChars = saved.numChars
	start.p[side].t_selected = main.f_tableCopy(saved.selected or {})
	start.p[side].t_selTemp = {}
	start.p[side].selEnd = true
	start.p[side].teamEnd = true
	for member, selection in ipairs(start.p[side].t_selected) do
		selection.pn = start.f_getPlayerNo(side, member)
		if selection.ref ~= nil then
			t_reservedChars[side][selection.ref] = true
		end
	end
	start.rankedCarry = nil
end
...
function start.f_palMenu(side, cmd, player, member, selectState)
	...
	if autoConfirm or getInput(cmd, motif.select_info['p' .. side].palmenu.done.key) or timerSelect == -1 then
		...
		if done_anim ~= preview_anim or done_spr[1] ~= -1 then
			if (st.face_anim ~= done_anim or done_spr[1] ~= -1) and ((main.coop and not motif.select_info.coopqueue) or motif.select_info['p' .. side].face.num > 1 or main.f_tableLength(start.p[side].t_selected) + 1 == start.p[side].numChars) then
		...
		if st.face2_anim ~= done_anim2 and ((main.coop and not motif.select_info.coopqueue) or motif.select_info['p' .. side].face2.num > 1 or main.f_tableLength(start.p[side].t_selected) + 1 == start.p[side].numChars) then
		...
		if selectState == 0 then
			...
			if start.p[side].t_selTemp[member] == nil then
				...
				if canConfirm then
					...
					local canShow = (main.coop and not motif.select_info.coopqueue) or motif.select_info['p' .. side].face.num > 1 or main.f_tableLength(start.p[side].t_selected) + 1 == start.p[side].numChars
					local canShow2 = (main.coop and not motif.select_info.coopqueue) or motif.select_info['p' .. side].face2.num > 1 or main.f_tableLength(start.p[side].t_selected) + 1 == start.p[side].numChars
...
local function onlineTeamModeName(mode)
	if mode == 1 then return 'simul' end
	if mode == 2 then return 'turns' end
	if mode == 3 then return 'tag' end
	return 'single'
end

-- Capture the selections after VS/order selection has finished. Character refs are
-- used deliberately: the synchronization/content fingerprint guarantees that the
-- same roster is being interpreted on both peers, while refs preserve exact team
-- identity even when the displayed name is not unique.
function start.f_captureOnlineRematchSnapshot()
	local snapshot = {
		version = 1,
		stageNo = getStageNo(),
		p1teammode = onlineTeamModeName(start.p[1].teamMode),
		p2teammode = onlineTeamModeName(start.p[2].teamMode),
		p1 = {},
		p2 = {},
	}
	for side = 1, 2 do
		local out = snapshot['p' .. side]
		...
		for slot, src in ipairs(remap) do
			local selected = start.p[side].t_selected[src]
	...
	start.onlineLastFightSnapshot = snapshot
	return snapshot
end

function start.f_prepareOnlineRematchSnapshot(snapshot)
	if type(snapshot) ~= 'table' then return nil end
	if nakama == nil or nakama.rankedSet == nil then return snapshot end
	local rs = nakama.rankedSet()
	-- A ranked side-switch is applied by start.f_game() after the previous
	-- gameplay session closes. The captured snapshot is physical-side based, so
	-- swap the two teams here to keep each set player's selection attached to
	-- their newly assigned side.
	if type(rs) == 'table' and rs.active and rs.switch_sides and not start.rankedSetFinished then
		return {
			version = snapshot.version,
			stageNo = snapshot.stageNo,
			p1teammode = snapshot.p2teammode,
			p2teammode = snapshot.p1teammode,
			p1 = main.f_tableCopy(snapshot.p2 or {}),
			p2 = main.f_tableCopy(snapshot.p1 or {}),
		}
	end
	return snapshot
end

function start.f_clearOnlineRematchState()
	start.onlineNextAction = nil
	start.onlineRematchSnapshot = nil
	start.onlineForfeitUserId = nil
end
...
function start.f_selectVersus(active, t_orderSelect, loadStartArg)
	...
	if not escFlag and gameMode('netplayversus') then
		start.f_captureOnlineRematchSnapshot()
	end
	return not escFlag
end
